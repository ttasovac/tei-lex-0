...
<p:declare-step xmlns:p="http://www.w3.org/ns/xproc"
    ...
    <!-- ================================================================== -->
    <!-- BODY: -->
    <p:directory-list name="dl"
                      path="../odd/examples"
                      include-filter=".+\.xml$"
                      exclude-filter=".+stripped\.xml$"
                      max-depth="unbounded" />
    <p:make-absolute-uris name="abs"
                          match="@name" />
    <p:variable name="input-file-count"
                select="count(//c:file)" />
    <p:for-each name="tei-stripper">
        ...
        <p:variable name="out"
                    select="replace($in, '\.xml$', '.stripped.xml', 'i')" />
        <p:load href="{$in}"
                content-type="application/xml" />
        <p:xslt>
            <p:with-input port="stylesheet"
                          href="../xslt/tei-stripper.xsl" />
        </p:xslt>
        <p:store name="store-it"
                 href="{$out}" />
    </p:for-each>
    ...
    <lex0:debug-store name="debug-include">
        <p:with-option name="name"
                       select="'include'" />
        <p:with-option name="debug"
                       select="$debug" />
        <p:with-input>
            <p:pipe step="do-include"
                    port="result" />
        </p:with-input>
    </lex0:debug-store>
    <p:xslt name="odd2odd">
        <p:with-input port="source">
            <p:pipe step="debug-include"
                    port="result" />
        </p:with-input>
        <p:with-input port="stylesheet">
    ...
    <lex0:debug-store name="debug-odd2lite">
        ...
        <p:with-option name="debug"
                       select="$debug" />
        <p:with-input>
            <p:pipe step="fix-odd2lite-used-by-classes"
                    port="result" />
        </p:with-input>
    </lex0:debug-store>
    <p:xslt name="fix-spec">
        <p:with-input port="source">
            <p:pipe step="debug-odd2lite"
                    port="result" />
        </p:with-input>
        <p:with-input port="stylesheet">
    ...
    <p:xslt name="odd2html"
        ...
        <p:with-input port="source">
            <p:pipe step="debug-expand-intro"
                    port="result" />
        </p:with-input>
        <p:with-input port="stylesheet">
