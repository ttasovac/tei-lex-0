...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!--This is a bit of a hack to force the creation of xml:ids in for each section
    in the specs: Elements, xx etc.
    I need those because otherwise a complex layout with splitLevel set to 2 or 3
    ...
    <xsl:template match="div[@xml:id = 'specification']">
        <xsl:element name="div" namespace="http://www.tei-c.org/ns/1.0">
            ...
            <xsl:copy-of
                select="document('../tei/parts/additions/specification.xml')/tei:TEI/tei:text/tei:body/node()"/>
            <xsl:apply-templates/>
        </xsl:element>
